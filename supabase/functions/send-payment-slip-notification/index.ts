...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const slips: NotificationRequest[] = body.slips || (body.slipId ? [body] : []);
    const portalUrl = body.portalUrl || "https://turakalnionamai.lovable.app";

    if (slips.length === 0) {
      console.log("No slips to notify about");
      return new Response(
        JSON.stringify({ success: true, sent: 0, message: "Nėra lapelių pranešimui" }),
    ...
    let residentsWithEmail: Map<string, { email: string; name: string }> = new Map();
    ...
    if (residentIds.length > 0) {
      ...
      if (residents) {
        for (const resident of residents) {
          ...
          if (resident.linked_profile_id && residentsWithEmail.has(resident.linked_profile_id)) {
            continue;
          }
    ...
    for (const slip of slips) {
      ...
      try {
        console.log(`Sending notification to ${recipient.email} for slip ${slip.slipId}`);
        ...
        if (emailResponse.ok) {
          ...
          console.error(`Failed to send email to ${recipient.email}:`, emailResult);
